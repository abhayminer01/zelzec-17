...
const registerUser = async (req, res) => {
  try {
    const { email, password, mobile, full_name, address, location, googleId } =
      req.body;
    if (!email || !password) {
      return res
        .status(400)
    ...
    transporter.sendMail(otpMailOptions, (err) => {
      if (err) console.error("Verification email error:", err);
    });
...
const deleteUser = async (req, res) => {
  try {
    ...
    if (user && user.email) {
      ...
      transporter.sendMail(deletionMailOptions, (err) => {
    ...
    req.session.destroy((err) => {
      if (err) {
        return res
          ...
          .json({
            success: false,
            message: "Account deletion scheduled but failed to log out",
          });
      }
